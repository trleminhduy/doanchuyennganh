...
function get_all_products()
{
    ...
    // Get the current limit from the URL if set
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : $initialLimit;

    // condition to check isset or not
    if (!isset($_GET['category']) && !isset($_GET['brand'])) {

        $select_query = "SELECT * FROM `products` ORDER BY rand() LIMIT $limit";
        $result_query = mysqli_query($con, $select_query);
    ...
    if ($limit == $initialLimit) {
        echo "<a href='?limit=" . ($limit + $initialLimit) . "' class='btn btn-info'>Hiển thị thêm</a>";
    } else {
        echo "<a href='?limit=$initialLimit' class='btn btn-info'>Hiển thị bớt</a>";
    }
    echo "</div>";
}
